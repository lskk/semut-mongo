...
class Maps extends MY_Controller{
    ...
    function get_gpstracer(){
        $connection = new MongoClient($this->Mserver);
        $collection = $connection->bsts_new->tb_tracker;
    ...
    function get_places(){
        $type=(int)$_GET['type'];
        $connection = new MongoClient($this->Mserver);
        $collection = $connection->bsts_new->tb_place;
        $document = $collection->find(array('TypeID' =>$type));
        $placestypes=$connection->bsts_new->tb_place_type;

        $places=array();
        foreach ($document as $id => $value )
        {
            $place['ID']=$value['ID'];
            $place['lat']=$value['Latitude'];
            $place['lon']=$value['Longitude'];
            $place['name']=$value['Name'];
            $place['address']=$value['Address'];
            $place['description']=$value['Description'];
            $placestype = $placestypes->findOne(array('ID' =>  $value['ID']), array('Name'));
            $place['type']=$placestype['Name'];
    ...
    function get_cctv(){
        ...
        $connection = new MongoClient($this->Mserver);
        $collection = $connection->bsts_new->tb_cctv;
        $cities= $connection->bsts_new->tb_city;
        $provincies= $connection->bsts_new->tb_province;
        $countries= $connection->bsts_new->tb_country;
        ...
        foreach ( $document as $id => $value )
        {
            ...
            if($value['CityID'] ==2){
                ...
                $cctv['Video']      = "http://bsts-svc.pptik.id/247/content/getcontent.php?type=video&vid=".$value['ItemID'];
                $cctv['Screenshot'] = "http://bsts-svc.pptik.id/247/content/getcontent.php?type=image&vid=".$value['ItemID'];

            }
            $cctv['Latitude']   =$value['Latitude'];
            $cctv['Longitude']  =$value['Longitude'];
            $city = $cities->findOne(array('ID' => $value['CityID']), array('Name','ProvinceID'));
            $cctv['City']       = $city["Name"];
            $province = $provincies->findOne(array('ID' =>  $city["ProvinceID"]), array('Name','CountryID'));
            $cctv['Province']   = $province['Name'];
            $country = $countries->findOne(array('ID' =>  $province["CountryID"]), array('Name'));
